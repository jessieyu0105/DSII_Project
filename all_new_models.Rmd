...
grid.arrange(pdp_rf_1, pdp_rf_2, pdp_rf_3, pdp_rf_4, pdp_rf_5, pdp_rf_6, nrow = 2)
...
pdp_gbm_6 <- gbm.fit4 %>%     # selected
  partial(pred.var = "age_19_64", 
          grid.resolution = 100) %>%
  autoplot(rug = TRUE, train = x1) +
  labs(x = "Percent with Age 19-64",
       y = "Partial Dependence Function")


grid.arrange(pdp_gbm_1, pdp_gbm_2, pdp_gbm_3, pdp_gbm_4, pdp_gbm_5,pdp_gbm_6, nrow = 2)
...
[ICE interpretation example](https://christophm.github.io/interpretable-ml-book/pdp.html)
...
grid.arrange(poverty_ice, poverty_ice2, nrow = 1)
```
...
# Plot
library(lime)
# select the 5th county
new_obs3 <- data_county_test[10, ] %>% 
  column_to_rownames("county") # row.names(new_obs) <- new_obs$county


explainer.gbm <- lime(data.frame(x1), gbm.fit4) # lime(predictors, model.fit)
explanation.gbm <- explain(new_obs3, explainer.gbm, n_features = 8) # n_features可用 8 or 5
rf_3 = plot_features(explanation.gbm)

explainer.rf <- lime(data.frame(x1), rf.fit) # lime(predictors, model.fit)
explanation.rf <- explain(new_obs3, explainer.gbm, n_features = 8) # n_features可用 8 or 5
gbm_3 = plot_features(explanation.rf)

grid.arrange(rf_3, gbm_3, nrow = 1)

```

In this county (Clarke AL), if poverty > 16.3, then it has a negative effect on the prediction of ALE using boosting
