...
rf_pdp = grid.arrange(pdp_rf_1, pdp_rf_2, pdp_rf_3, pdp_rf_4, pdp_rf_5, pdp_rf_6, nrow = 2)
...
gbm_pdp = grid.arrange(pdp_gbm_1, pdp_gbm_2, pdp_gbm_3, pdp_gbm_4, pdp_gbm_5,pdp_gbm_6, nrow = 2)
...
new_obs <- data_county_test[5, ] %>% 
  column_to_rownames("county") # row.names(new_obs) <- new_obs$county


explainer.gbm <- lime(data.frame(x1), gbm.fit) # lime(predictors, model.fit)
explanation.gbm <- explain(new_obs, explainer.gbm, n_features = 8) # n_features可用 8 or 5
